        ...
        <el-dialog v-model="editDialogVisible" title="编辑菜单信息" width="50%">
            <el-form :model="editForm" label-width="100px">
                <el-form-item label="菜单ID" required>
                    <el-input v-model="editForm.menuId" placeholder="请输入菜单ID" disabled></el-input>
                </el-form-item>
...
const getSearchList = () => {
    searchMenuCopy(searchForm.value, paginationForm.value).then((res) => {
...
const handleDelete = (row) => {
    ElMessageBox.confirm(`确定要删除菜单【${row.title}】吗？`, '提示', {
        ...
        console.log({ id: row.menuId })
        deleteSelectMenuCopy({ id: row.menuId }).then(res => {
            if (res.code === 0) {
...
const submitAdd = () => {
    ElMessageBox.confirm('确定要保存修改吗？', '提示', {
        ...
        addMenuCopy(addForm.value).then(res => {
            if (res.code === 0) {
...
const submitEdit = () => {
    ElMessageBox.confirm('确定要保存修改吗？', '提示', {
        ...
        editSelectMenuCopy(editForm.value).then(res => {
            if (res.code === 0) {
                ElMessage.success('修改成功')
